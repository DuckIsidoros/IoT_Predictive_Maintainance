...
const char *CURRENT_LABEL = "imbalanced";
...
void setup()
{
    Serial.begin(921600); // To avoid bottleneck when collecting data;
    while (!Serial)
        ;
    ...
    timerAlarm(timer, 2000, true, 0); // Khóa cứng chu kỳ 2000 us = 2ms.
}
...
void loop()
{
    if (samplingTriggered)
    {
        ...
        if (IMU_ReadAccel(raw_counts))
        {
            ...
            // Chỉ in các cột Raw cho 3 trục, và thêm 1 cột Z đã lọc cho FFT
            ...
            Serial.print(","); // Cột riêng cho FFT
        ...
        else
        {
            ...
            if (error_counter > 20)
            {
                ...
                // Giới hạn tần suất Re-init, chỉ cho phép thử lại sau mỗi 100ms
                // để tránh việc spam lệnh I2C liên tục gây nghẽn loop
                if (current_time_ms - last_recovery_time_ms > 100)
                {
                    last_recovery_time_ms = current_time_ms;

                    // Giải pháp dứt điểm: Giải phóng bus và ép phần cứng I2C khởi động lại
                    Wire.end();

                    // Khởi động lại không sử dụng hàm delay() blocking
                    Wire.begin(I2C_SDA, I2C_SCL);
                    Wire.setClock(400000);

                    // Gửi lệnh đánh thức MPU6050
                    Wire.beginTransmission(IMU_ADDRESS);
                    Wire.write(IMU_REG_PWR_MGMT_1);
                    Wire.write(0x00);
                    Wire.endTransmission();

                    // Cấu hình lại dải đo ±16g
                    Wire.beginTransmission(IMU_ADDRESS);
                    Wire.write(IMU_REG_ACCEL_CONFIG);
                    Wire.write(0x18);
                    Wire.endTransmission();

                    error_counter = 0; // Reset bộ đếm lỗi để thử lại ở chu kỳ sau
                }
            }
        }
    }
