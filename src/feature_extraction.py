...
def calculate_crest_factor(values: np.ndarray, rms_value: float) -> float:
    """
    Calculate Crest Factor (Yếu tố đỉnh).
    Crest Factor = Peak / RMS
    """
    if rms_value <= 0:
        return 0.0
    peak_value = np.max(np.abs(values))
    return float(peak_value / rms_value)


def calculate_kurtosis(values: np.ndarray) -> float:
    """
    Calculate Kurtosis (Độ nhọn phổ/tín hiệu).
    Đo lường mức độ đột biến (va đập) của chuỗi số liệu.
    """
    if len(values) < 4: # Tránh lỗi chia cho 0 khi số mẫu quá ít
        return 0.0
    # Thiết lập fisher=True để phân phối chuẩn có Kurtosis = 0
    return float(scipy_kurtosis(values, fisher=True, nan_policy='omit'))
...
def extract_time_domain_features_from_window_file(
    ...
    if len(acc_z) != WINDOW_SIZE:
        raise ValueError(
            f"Window file {window_file_path.name} is corrupted. "
            f"Got {len(acc_z)} valid samples, expected exact {WINDOW_SIZE}."
        )
    ...
    rms_z_detrend = calculate_rms(acc_z_detrend)

    time_features = {
        "RMS_X": calculate_rms(acc_x),
        "RMS_Y": calculate_rms(acc_y),
        "RMS_Z": calculate_rms(acc_z),
        "CrestFactor_Z": calculate_crest_factor(acc_z_detrend, rms_z_detrend),
        "Kurtosis_Z": calculate_kurtosis(acc_z_detrend),
    }
...
def extract_features_from_fft_file(file_path: Path) -> tuple[dict, dict]:
    ...
    fft_features = {
        ...
        "Band_Power_Z_Low": band_power_low,
        "Band_Power_Z_Mid": band_power_mid,
        "Band_Power_Z_High": band_power_high,
        "label": label,
    }
...
def extract_hybrid_features_from_fft_file(
    ...
    feature_row = {
        ...
        "Band_Power_Z_Low": fft_features["Band_Power_Z_Low"],
        "Band_Power_Z_Mid": fft_features["Band_Power_Z_Mid"],
        "Band_Power_Z_High": fft_features["Band_Power_Z_High"],

        "CrestFactor_Z": time_features["CrestFactor_Z"],
        "Kurtosis_Z": time_features["Kurtosis_Z"],
